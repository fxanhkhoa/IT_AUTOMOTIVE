...
def callback_ReadImage(input):
    #print(input.format)
    np_arr = np.fromstring(input.data, np.uint8)
...
def main():
    ...
    subscriber = rospy.Subscriber("/Team1_image/compressed", CompressedImage, callback_ReadImage)
    rospy.spin()
    #threadLane.start()
    #threadSign.start()
